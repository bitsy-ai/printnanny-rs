...
impl PrintNannyPipelineFactory {
    ...
    async fn make_jpeg_snapshot_pipeline(
        ...
        let description = format!("interpipesrc name={interpipesrc} listen-to={listen_to} accept-events=false accept-eos-event=false is-live=true allow-renegotiation=false max-buffers=2 leaky-type=2 do-timestamp=true \
    ...
    async fn make_h264_pipeline(
        ...
        let description = format!("interpipesrc name={interpipesrc} listen-to={listen_to} accept-events=false accept-eos-event=false is-live=true allow-renegotiation=false do-timestamp=true \
            ! v4l2convert \
